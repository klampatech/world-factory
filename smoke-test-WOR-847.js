...
const SCREENSHOT_DIR = 'screenshots/smoke-test-WOR-847/';
const screenshots = [];
...
async function testFrontend(worldId) {
  ...
  try {
    if (worldId) {
      ...
      await page.goto(`${FRONTEND_URL}/worlds/${worldId}/timeline`);
      await page.waitForLoadState('networkidle');
      await page.waitForTimeout(2000);
      ...
      await page.goto(`${FRONTEND_URL}/worlds/${worldId}/dashboard`);
      await page.waitForLoadState('networkidle');
      await page.waitForTimeout(1000);
      ...
      await page.goto(`${FRONTEND_URL}/worlds/${worldId}/figures`);
      await page.waitForLoadState('networkidle');
      await page.waitForTimeout(1000);
      ...
      await page.goto(`${FRONTEND_URL}/worlds/${worldId}`);
      await page.waitForLoadState('networkidle');
...
async function main() {
  console.log('╔════════════════════════════════════════════════════════════╗');
  console.log('║         WORLD FACTORY SMOKE TEST - WOR-847            ║');
  console.log('╚════════════════════════════════════════════════════════════╝');
  ...
  const report = `# WOR-847 Smoke Test Report
...
`;
  ...
  writeFileSync('WOR-847-SMOKE-TEST-REPORT.md', report);
  console.log('\n📄 Report saved to WOR-847-SMOKE-TEST-REPORT.md');
