...
const SCREENSHOT_DIR = '/home/kyle/projects/world-generator/screenshots/smoke-WOR-1223';
const REPORT_FILE = '/home/kyle/projects/world-generator/WOR-1223-SMOKE-TEST-REPORT.json';
...
async function run() {
  console.log('╔══════════════════════════════════════════════════════════════╗');
  console.log('║         WOR-1223 Smoke Test - Full End-to-End                ║');
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    if (title.includes('World') || title.includes('Factory')) {
      results.frontend.passed++;
      ...
      results.frontend.failed++;
      results.frontend.tests.push({ name: 'Homepage loads with correct title', passed: false, title });
      log('FAIL', 'Frontend', `Homepage title unexpected: "${title}"`);
    }
  } catch (e) {
    results.frontend.failed++;
  ...
  if (criticalErrors.length === 0) {
    results.frontend.passed++;
    results.frontend.tests.push({ name: 'No critical console errors', passed: true });
    log('PASS', 'Frontend', 'No critical console errors');
  } else {
    results.frontend.failed++;
    results.frontend.tests.push({ name: 'No critical console errors', passed: false, count: criticalErrors.length });
  ...
  if (results.api.failed > 0 || results.frontend.failed > 0 || criticalErrors.length > 0) {
    console.log('\n❌ SMOKE TEST FAILED\n');
  } else {
    console.log('\n✅ SMOKE TEST PASSED\n');
  }
  ...
  // Print failed tests
  const failedAPI = results.api.tests.filter(t => !t.passed);
  const failedFE = results.frontend.tests.filter(t => !t.passed);
  
  if (failedAPI.length > 0) {
    console.log('Failed API tests:');
    failedAPI.forEach(t => console.log(`  - ${t.name} (${t.method} ${t.path}) → ${t.status || 'ERROR'}`));
  }
  
  if (failedFE.length > 0) {
    console.log('Failed Frontend tests:');
    failedFE.forEach(t => console.log(`  - ${t.name}`));
  }
  ...
  process.exit(results.api.failed > 0 || results.frontend.failed > 0 ? 1 : 0);
}
