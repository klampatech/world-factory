...
use world_factory::{
    World, TerrainGenerator, TerrainConfig, TerrainLayer,
    HistoryGenerator, GeneratorConfig,
    figures::FigureStore,
...
fn test_phase2_integration_500_years() {
    ...
    println!("[3/9] Running HistoryGenerator ({} years)...", PRE_HISTORY_YEARS);
    ...
    println!("   Stats: {} events, {} figures, {} artifacts",
        result.stats.event_count,
        result.stats.figure_count,
        result.stats.artifact_count);
    ...
    assert!(event_count >= MIN_EVENTS, 
        "Expected ≥{} events, got {}", MIN_EVENTS, event_count);
    ...
    println!("   ✓ Timeline sorted: {} to {}", 
        events.first().map(|e| get_year(&e.time)).unwrap_or(0), last_year);
    ...
    if figure_count >= MIN_FIGURES {
        ...
        let figures_with_bios: usize = figures.iter()
            .filter(|f| f.biography.is_some() && !f.biography.as_ref().unwrap().is_empty())
            .count();
        println!("   Figures with biographies: {}", figures_with_bios);
        assert!(figures_with_bios >= MIN_FIGURES,
            "Expected ≥{} figures with biographies, got {}", MIN_FIGURES, figures_with_bios);
    ...
    for figure in &figures {
        if let (Some(birth), Some(death)) = (figure.birth_year, figure.death_year) {
            if birth >= death {
                println!("   ✗ Figure {} invalid lifecycle: birth={}, death={}", 
                    figure.id, birth, death);
                all_valid = false;
            }
        }
    }
    assert!(all_valid, "Some figures have invalid lifecycle (birth >= death)");
    println!("   ✓ All figure lifecycles valid");
    
    // Criterion 3: Artifacts created (at least 1)
    println!("\n[8/9] Verifying artifacts (≥{} required)...", MIN_ARTIFACTS);
    let artifacts = artifact_store.artifacts();
    let artifact_count = artifacts.len();
    println!("   Artifacts found: {}", artifact_count);
    assert!(artifact_count >= MIN_ARTIFACTS,
        "Expected ≥{} artifacts, got {}", MIN_ARTIFACTS, artifact_count);
    ...
    assert!(cataclysms <= MAX_CATACLYMS,
        "Expected ≤{} cataclysms, got {}", MAX_CATACLYMS, cataclysms);
    ...
    let war_events: Vec<_> = events.iter()
        ...
        for war in &war_events {
            ...
            if let Some(ref participants) = participants {
                if !participants.is_empty() {
                    ...
                    println!("   War at year {}: {} participants", year, participants.len());
                }
            }
        }
    ...
    println!("\n   Artifact conditions validated (figures: {}, resources implied)", figure_count);
...
fn test_phase2_determinism() {
    ...
    // Run 1
    let result1 = HistoryGenerator::with_config(config.clone(), Some(seed))
    ...
    println!("Run 1: {} events, {} figures", events1.len(), figures1.len());
    ...
    if !events1.is_empty() && !events2.is_empty() {
        ...
        assert_eq!(year1, year2,
            "First event year differs - not deterministic");
...
fn test_phase2_artifact_conditions() {
    ...
    for artifact in artifacts {
        println!("  Artifact: {} (category: {:?})", artifact.name, artifact.category);
        
        // Verify creation year is reasonable
        assert!(artifact.created_year >= 0, "Artifact created_year is negative");
        assert!(artifact.created_year <= PRE_HISTORY_YEARS, 
