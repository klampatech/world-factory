...
use world_factory::{
    generate_voronoi_graph, BiomeAssignmentMatrix, BiomeType, OceanDetectionConfig, OceanDetector,
    OceanZone, PolygonGraph, Species, SpeciesId, TerrainCell, TerrainConfig, TerrainGenerator,
    TerrainGrid, TerrainLayer, VoronoiConfig, VoronoiGenerator, WorldConfig,
};
...
fn test_world_generation_voronoi_polygons() {
    ...
    assert!(polygon_count > 0, "No Voronoi polygons generated");
    ...
    assert_eq!(
        polygons_with_elevation, polygon_count,
        "All polygons should have elevation values"
    );
...
fn test_world_generation_settlements() {
    ...
    println!(
        "Voronoi polygons generated: {} (settlement placement requires species setup)",
...
fn test_world_generation_determinism() {
    ...
    println!(
        "Generation time difference: {}ms (non-critical metric)",
        time_diff
    );
...
fn test_world_generation_complete_e2e() {
    ...
    if let Some(ref result) = world.tectonic_result {
        assert!(
            !result.plates.is_empty() || result.boundaries.is_empty(),
