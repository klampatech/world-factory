...
fn cli_world_factory_dir_env_respected() {
    ...
    if output.status.success() {
        let has_path_hint = stderr.contains("world") || 
                           stderr.contains("saved") || 
                           stderr.contains("data");
        println!("Custom dir test - output mentions storage: {}", has_path_hint);
    }
...
fn cli_default_storage_when_env_not_set() {
    ...
    assert!(output.status.success() || combined > 0,
            "Should produce output even without custom storage dir");
    
    println!("Default storage test completed. Output length: {}", combined);
}
...
fn cli_server_shared_storage() {
    ...
    println!("CLI with WORLD_FACTORY_DIR: success={}, stderr_len={}", 
             cli_output.status.success(), cli_stderr_len);
...
fn cli_invalid_world_factory_dir() {
    ...
    cmd.env("WORLD_FACTORY_DIR", "/nonexistent/path/that/does/not/exist/anywhere");
    cmd.stdout(Stdio::piped());
    cmd.stderr(Stdio::piped());
    ...
    println!("Invalid WORLD_FACTORY_DIR test: exit_code={}, output_len={}", 
             output.status.code().unwrap_or(-1), combined_len);
}
