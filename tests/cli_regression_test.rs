...
fn cli_h_flag_conflict_regression() {
    ...
    let gen_help_output = gen_help_cmd
        .output()
        .expect("Failed to execute generate --help");
    ...
    assert!(
        gen_help.contains("seed") || gen_help.contains("width") || gen_help.contains("height"),
    ...
    let height_output = height_cmd
        .output()
        .expect("Failed to execute generate with --height");
    let height_stderr = String::from_utf8_lossy(&height_output.stderr);
    assert!(
        height_output.status.success(),
        "--height should work unambiguously. stderr: {}",
        height_stderr
    );
