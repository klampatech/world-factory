...
use world_factory::history::society::{Society, SocietyType, SocietyRegistry};
use world_factory::events::timeline::EventTimeline;
use world_factory::events::{Event, EventType, EventBuilder};
use world_factory::events::probability::{ProbabilityEngine, EventContext};
use world_factory::figures::{NotableFigure, FigureType, FigureLifecycleState};
use world_factory::artifacts::{Artifact, ArtifactCategory, ArtifactRarity};
use world_factory::types::HistoricalTime;
use world_factory::terrain::{Polygon, OceanDetector};
...
fn test_settlement_not_in_ocean() {
    ...
    assert!(detector.is_ocean(&ocean_polygon),
        "Polygon with elevation -0.3 should be ocean");
    
    // Land polygon should not be ocean
    assert!(!detector.is_ocean(&land_polygon),
    ...
    assert!(detector.is_ocean(&coast_polygon),
    ...
    assert!(detector.is_ocean(&very_shallow),
        "Very shallow ocean (-0.1) should be detected as ocean");
    assert!(detector.is_ocean(&very_deep),
    ...
    assert!(!detector.is_ocean(&highland),
    ...
    assert!(!detector.is_ocean(&just_above),
        "Just above sea level (0.0001) should NOT be ocean");
    assert!(detector.is_ocean(&just_below),
...
fn test_population_logistic_growth() {
    ...
    if tick_results.len() >= 10 {
        let early_avg_growth: f32 = tick_results[..5].iter()
            .map(|r| r.growth_rate)
            .sum::<f32>() / 5.0;
        
        let late_avg_growth: f32 = tick_results[tick_results.len() - 5..].iter()
            .map(|r| r.growth_rate)
            .sum::<f32>() / 5.0;
        
        // Growth rate should decrease over time (logistic curve behavior)
        assert!(early_avg_growth > late_avg_growth * 0.5,
            "Early growth rate ({}) should exceed late growth rate ({}) for logistic growth",
            early_avg_growth, late_avg_growth);
    }
}
...
fn test_society_hierarchy_transitions() {
    ...
    assert!(society.check_transition().is_none(),
    ...
    assert_eq!(old_type, Some(SocietyType::Tribe),
    ...
    assert_eq!(old_type, Some(SocietyType::Chiefdom),
    ...
    assert!(SocietyType::Nation.evolve_to().is_none(),
    ...
    assert_eq!(old_type, Some(SocietyType::Nation),
...
fn test_figure_birth_before_death() {
    ...
    assert!(lifespan1.is_some(), "Figure with both dates should have lifespan");
    ...
    assert!(lifespan5.is_none(), "Same year (death <= birth) should return no lifespan");
...
fn test_artifact_creation_requires_figure() {
    ...
    assert!(artifact1.creator_id.is_some(),
        "Artifact with creator should have creator_id");
    assert_eq!(artifact1.creator_id.unwrap(), creator_figure_id,
    ...
    assert!(artifact2.creator_id.is_none(),
    ...
    assert!(legendary_artifact.significance > 0.8,
        "Legendary artifacts should have high significance");
...
fn test_history_timeline_chronological() {
    ...
    assert_eq!(year_range.unwrap(), (1000, 1200),
    ...
    assert_eq!(settlements.len(), 1, "Should have exactly 1 settlement founding");
    
    // Test first/last events
    assert!(timeline.first().is_some(), "Timeline should have first event");
    assert!(timeline.last().is_some(), "Timeline should have last event");
    assert_eq!(timeline.first().unwrap().time.get_year(), 1000,
        "First event should be year 1000");
    assert_eq!(timeline.last().unwrap().time.get_year(), 1200,
        "Last event should be year 1200");
    
    // Test timeline stats
    let stats = timeline.stats();
    assert_eq!(stats.total_events, 5, "Stats should show 5 events");
    assert!((stats.average_significance - 0.7).abs() < 0.01,
    ...
    assert!(same_year_events[0].significance.unwrap() > same_year_events[1].significance.unwrap(),
