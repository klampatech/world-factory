...
use world_factory::{
    ...
    TerrainGrid,
    PolygonGraph, VoronoiConfig, generate_voronoi_graph,
...
fn test_terrain_generation_basic() {
    ...
    println!("Terrain generated: {}x{} grid, elevation range: {:.0}m to {:.0}m",
...
fn test_ocean_detection_basic() {
    ...
    for (_, _, zone) in &ocean_zones {
        match zone {
            ...
            OceanZone::MediumOcean => { /* counts as shallow */ shallow_count += 1; }
        }
    }
    
    println!("Ocean detection: {} land, {} shallow, {} deep",
             land_count, shallow_count, deep_count);
    
    // At least some cells should be land or ocean
    assert!(land_count > 0 || shallow_count > 0, "No ocean zones detected");
}
...
fn test_terrain_determinism() {
    ...
    // Compare cell data
    let cells1: Vec<_> = grid1.cells().map(|(_, _, c)| (c.height(), c.moisture())).collect();
    let cells2: Vec<_> = grid2.cells().map(|(_, _, c)| (c.height(), c.moisture())).collect();
...
fn test_voronoi_determinism() {
    ...
    for id in ids1 {
        ...
        assert_eq!(p1.elevation, p2.elevation, "Polygon elevation differs for id {:?}", id);
    }
...
fn test_performance_small() {
    ...
    assert!(elapsed < SMALL_GENERATION_LIMIT,
        "Small world took {:.2}s, limit {:.1}s", elapsed, SMALL_GENERATION_LIMIT);
    
    println!("Small world ({}x{}): {:.3}s", SMALL_SIZE, SMALL_SIZE, elapsed);
}
...
fn test_performance_medium() {
    ...
    assert!(elapsed < MEDIUM_GENERATION_LIMIT,
        "Medium world took {:.2}s, limit {:.1}s", elapsed, MEDIUM_GENERATION_LIMIT);
    
    println!("Medium world ({}x{}): {:.3}s", MEDIUM_SIZE, MEDIUM_SIZE, elapsed);
}
...
fn test_performance_large() {
    ...
    assert!(elapsed < LARGE_GENERATION_LIMIT,
        "Large world took {:.2}s, limit {:.1}s", elapsed, LARGE_GENERATION_LIMIT);
    
    println!("Large world ({}x{}): {:.3}s", LARGE_SIZE, LARGE_SIZE, elapsed);
}
...
fn test_complete_pipeline_with_tectonics() {
    ...
    if let Some(ref tectonic) = world.tectonic_result {
        assert!(!tectonic.plates.is_empty() || tectonic.boundaries.is_empty(),
            "Tectonic plates should exist when enabled");
    }
    
    println!("Complete pipeline: terrain + {} polygons + tectonic",
...
fn test_climate_calculation() {
    ...
    println!("Climate calculated: temperature range {:.1}°C to {:.1}°C",
             min_temp, max_temp);
}
...
fn test_biome_diversity_quality() {
    ...
    assert!(biomes.len() >= 3, "Biome diversity too low: {} types", biomes.len());
...
fn test_elevation_variation_quality() {
    ...
    assert!(std_dev >= 10.0, "Elevation variation too uniform: std_dev {:.1}", std_dev);
}
...
fn test_polygon_distribution_quality() {
    ...
    let elevations: Vec<f32> = world.polygon_graph.polygon_ids()
    ...
    if !elevations.is_empty() {
        ...
        println!("Polygon distribution: {} cells, elevation range {:.2} to {:.2}",
                 polygon_count, min_elev, max_elev);
    }
    
    // Polygons should have some variation in elevation
    assert!(polygon_count >= 200, "Too few polygons generated: {}", polygon_count);
}
