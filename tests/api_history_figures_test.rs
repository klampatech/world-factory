...
mod tests {
    ...
    use world_factory::types::{HistoricalTime, World, EntityId, EntityType, Timestamp};
    use uuid::Uuid;
    ...
    fn create_test_event(world_id: Uuid, year: i32, event_type: EventType, name: &str) -> world_factory::events::Event {
    ...
    fn create_test_figure(world_id: Uuid, figure_type: FigureType, significance: f32) -> world_factory::figures::NotableFigure {
    ...
    fn test_figure_type_filtering() {
        ...
        figures.push(create_test_figure(world_id, FigureType::MilitaryLeader, 0.85));
        
        // Test filtering
        let monarch_count = figures.iter().filter(|f| f.figure_type == FigureType::Monarch).count();
        assert_eq!(monarch_count, 1, "Should have exactly 1 monarch");
        
        let military_count = figures.iter().filter(|f| f.figure_type == FigureType::MilitaryLeader).count();
    ...
    fn test_world_package_with_events_and_figures() {
        ...
        package.event_store_events.push(create_test_event(
            world_id, 100, EventType::SettlementFounded, "First Settlement"
        ));
        package.event_store_events.push(create_test_event(
            world_id, 200, EventType::WarDeclared, "The Great War"
        ));
        package.event_store_events.push(create_test_event(
            world_id, 300, EventType::Battle, "Battle of Valor"
        ));
        ...
        println!("Created monarch: type={:?}, id={:?}", monarch.figure_type, monarch.id);
        println!("Created military: type={:?}, id={:?}", military.figure_type, military.id);
        ...
        let monarchs: Vec<_> = package.notable_figures.iter()
        ...
        let military: Vec<_> = package.notable_figures.iter()
        ...
        let events_100_250: Vec<_> = package.event_store_events.iter()
            .filter(|e| e.time.get_year() >= 100 && e.time.get_year() <= 250)
            .collect();
        assert_eq!(events_100_250.len(), 2, "Should have 2 events in year range 100-250");
    ...
    fn test_world_package_serialization_roundtrip() {
        ...
        // Add some events
        package.event_store_events.push(create_test_event(
            world_id, 500, EventType::CulturalAchievement, "Golden Age"
        ));
        
        // Add some figures
        package.notable_figures.push(create_test_figure(
            world_id, FigureType::Scholar, 0.75
        ));
    ...
    fn test_event_filtering_by_significance() {
        ...
        package.event_store_events.push(create_test_event(
            world_id, 100, EventType::SettlementFounded, "Minor Settlement"
        ));
        // Note: create_test_event uses 0.7 significance
        
        // Test that significance filtering works
        let high_significance: Vec<_> = package.event_store_events.iter()
