...
async function runSmokeTest() {
  console.log('=== WORLD FACTORY COMPREHENSIVE SMOKE TEST (WOR-1154) ===');
  ...
  const backendTests = [
    { 
      ...
      fn: async () => {
        const resp = await fetch(`${API_BASE}/worlds`, {
          ...
          body: JSON.stringify({ name: 'QA-Smoke-WOR1154', seed: 11541154, config: { genre: 'fantasy' } })
        });
    ...
    { name: '9. GET /api/v1/worlds/:id/figures/:id - Get specific figure (404 expected)', fn: async () => {
      ...
      return { status: resp.status, success: [200, 404].includes(resp.status) };
    }},
    ...
    { name: '18. GET /health - Health check', fn: async () => {
      const resp = await fetch('http://localhost:8082/health');
      return { status: resp.status, success: resp.status === 200 };
    }},
  ];
  ...
  page.on('console', msg => {
    if (msg.type() === 'error') {
      const text = msg.text();
      // Ignore favicon and common benign errors
  ...
  await page.screenshot({ path: '/home/kyle/projects/world-generator/screenshots/WOR-1154-1-home.png' });
  ...
  await page.screenshot({ path: '/home/kyle/projects/world-generator/screenshots/WOR-1154-2-canvas-check.png' });
  ...
  await page.screenshot({ path: '/home/kyle/projects/world-generator/screenshots/WOR-1154-3-dashboard.png' });
  ...
  fs.writeFileSync('/home/kyle/projects/world-generator/WOR-1154-SMOKE-TEST-REPORT.json', JSON.stringify(results, null, 2));
  
  const mdReport = `# WOR-1154 Smoke Test Report
...
`;
  fs.writeFileSync('/home/kyle/projects/world-generator/WOR-1154-SMOKE-TEST-REPORT.md', mdReport);
