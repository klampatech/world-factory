#!/usr/bin/env node
/**
 * WOR-1174 Smoke Test v2 - Full End-to-End Test (Fixed Navigation)
...
const SCREENSHOT_DIR = './screenshots/WOR-1174-v2';
...
async function closeAnyModal() {
  try {
    // Close any open modal by pressing Escape or finding close button
...
async function testFrontendUI() {
  ...
  // UI-03: Map view (navigate to a world first)
  console.log('[UI-03] Testing map view...');
  try {
    await closeAnyModal();
    
    // Find a world to click on
    const worldLink = page.locator('a[href*="world"], button:has-text("View Map"), button:has-text("Open"), [class*="world"]:not([class*="create"])').first();
    
    // Try to find a complete world or the first one
    ...
    if (mapVisible) {
      ...
      // Check for Voronoi (varied colors indicating polygons)
      ...
      // May be on world selector - check for map-related elements
