 ...
 */
function Dashboard({ initialWorlds, limit = 20, className, onWorldSelect, onCreateWorld, initialMetrics, }) {
    ...
    const handleWorldClick = (0, react_1.useCallback)((worldId) => {
    ...
    const handleCreateClick = (0, react_1.useCallback)(() => {
        onCreateWorld?.();
    }, [onCreateWorld]);
    ...
    // Loading state
    if (state.loading) {
        return (<div className={`dashboard ${className || ''}`}>
        <div className="dashboard-header">
          <h2>World Dashboard</h2>
        </div>
        <div className="dashboard-content">
          <div className="world-grid">
            {[1, 2, 3].map(i => (<div key={i} className="world-card skeleton">
    ...
    return (<div className={`dashboard ${className || ''}`}>
      ...
      {state.selectedWorldMetrics && (<div className="world-state-summary">
          ...
          <div className="summary-grid">
            ...
            <div className="metric-card">
              ...
              <div className="species-breakdown">
                {state.selectedWorldMetrics.populationBySpecies.slice(0, 4).map(sp => (<div key={sp.species} className="species-row">
            ...
            <div className="metric-card">
              ...
              <div className="resources-grid">
                {state.selectedWorldMetrics.resources.map(res => (<div key={res.type} className="resource-row">
                    <span className="resource-type">{res.type}</span>
                    <span className="resource-scarcity" style={{ color: getScarcityColor(res.scarcity) }}>
                      {res.scarcity}
                    </span>
      ...
      {/* World List */}
      <div className="world-grid">
        {state.worlds.map(world => (<div key={world.id} className="world-card" onClick={() => handleWorldClick(world.id)} role="button" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && handleWorldClick(world.id)}>
            <div className="world-card-header">
              <h3 className="world-name">{world.name}</h3>
              <span className="world-status" style={{ color: getStatusColor(world.status) }}>
                {getStatusLabel(world.status)}
              </span>
            </div>
            
            {world.status === 'generating' && (<div className="progress-container">
                <div className="progress-bar">
                  <div className="progress-fill" style={{ width: `${world.progress * 100}%` }}/>
                </div>
            ...
            <div className="world-card-meta">
              ...
              {world.parameters?.seed && (<span className="meta-item">
