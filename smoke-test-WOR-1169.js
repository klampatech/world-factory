...
const SCREENSHOTS_DIR = '/home/kyle/projects/world-generator/screenshots/WOR-1169';
...
async function runSmokeTest() {
  console.log('=== WORLD FACTORY SMOKE TEST (WOR-1169) ===');
    ...
    { 
      ...
      fn: async () => {
        const resp = await fetch(`${API_BASE}/worlds`, {
          ...
          body: JSON.stringify({ name: 'QA-Smoke-WOR1169', seed: 11691169, config: { genre: 'fantasy' } })
        });
  ...
  await page.screenshot({ path: `${SCREENSHOTS_DIR}/01-home-page.png` });
  
  const title = await page.title();
  const homeSuccess = title.includes('World') || title.includes('Factory') || title.includes('Procedural');
  ...
  console.log('  Testing map canvas...');
  const canvas = await page.locator('canvas').count();
  await page.screenshot({ path: `${SCREENSHOTS_DIR}/02-canvas-check.png` });
  ...
  // 7. Timeline/historical events loading
  const timelineLinks = await page.locator('text=/history|event|timeline/i').count();
  const timelineOk = timelineLinks > 0;
  results.frontend.tests.push({ name: 'Timeline/navigation elements present', passed: timelineOk, count: timelineLinks });
  results.frontend.passed += timelineOk ? 1 : 0;
  results.frontend.failed += timelineOk ? 0 : 1;
  console.log(`  ${timelineOk ? '✓' : '✗'} Timeline-related elements: ${timelineLinks}`);

  await browser.close();

  // === SUMMARY ===
  console.log('\n=== SMOKE TEST SUMMARY ===');
  console.log(`Issue: WOR-1169`);
  console.log(`Timestamp: ${results.timestamp}`);
  console.log(`\nBackend: ${results.backend.passed}/${results.backend.endpoints.length} endpoints passed`);
  if (results.backend.failed > 0) {
    console.log('  Failed endpoints:');
    results.backend.endpoints.filter(e => !e.passed).forEach(e => console.log(`    - ${e.name}`));
  }
  console.log(`\nFrontend: ${results.frontend.passed}/${results.frontend.tests.length} tests passed`);
  if (results.frontend.failed > 0) {
    console.log('  Failed tests:');
    results.frontend.tests.filter(e => !e.passed).forEach(e => console.log(`    - ${e.name}`));
  }
  console.log(`\nConsole errors: ${results.consoleErrors.length}`);
  if (results.consoleErrors.length > 0) {
    results.consoleErrors.forEach(e => console.log(`  - ${e.substring(0, 80)}`));
  }
  ...
  // Write reports
  fs.writeFileSync('/home/kyle/projects/world-generator/WOR-1169-SMOKE-TEST-REPORT.json', JSON.stringify(results, null, 2));
  
  const mdReport = `# WOR-1169 Smoke Test Report

## Summary
- **Status**: ${results.backend.passed === results.backend.endpoints.length && results.frontend.passed === results.frontend.tests.length && results.consoleErrors.length === 0 ? '✅ PASS' : '❌ FAIL'}
- **Timestamp**: ${results.timestamp}
- **Backend**: ${API_BASE}
- **Frontend**: ${FRONTEND_URL}

## Backend API Results (${results.backend.passed}/${results.backend.endpoints.length} passed)

| Endpoint | Status | Result |
|----------|--------|--------|
${results.backend.endpoints.map(e => `| ${e.name} | ${e.status || 'ERROR'} | ${e.passed ? '✅' : '❌'}`).join('\n')}

## Frontend UI Results (${results.frontend.passed}/${results.frontend.tests.length} passed)

| Test | Result |
|------|--------|
${results.frontend.tests.map(t => `| ${t.name} | ${t.passed ? '✅' : '❌'}`).join('\n')}

## Console Errors
${results.consoleErrors.length === 0 ? '✅ No Error-level console messages' : results.consoleErrors.map(e => `- ${e}`).join('\n')}

## Bugs Found
${results.bugs.length === 0 ? 'None' : results.bugs.map(b => `- **${b.endpoint}**: ${b.issue}`).join('\n')}
...
`;
  fs.writeFileSync('/home/kyle/projects/world-generator/WOR-1169-SMOKE-TEST-REPORT.md', mdReport);
