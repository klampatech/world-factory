...
const SCREENSHOT_DIR = 'screenshots/smoke-test-WOR-919/';
const screenshots = [];
...
async function testApiEndpoints() {
  ...
  try {
    const createResponse = await fetch(`${BASE_URL}/api/v1/worlds`, {
      ...
      body: JSON.stringify({
        name: 'WOR-919 Smoke Test World',
        config: { width: 32, height: 32, pre_history_years: 50, seed: 919001 }
      })
    });
...
async function main() {
  console.log('╔════════════════════════════════════════════════════════════╗');
  console.log('║         WORLD FACTORY SMOKE TEST - WOR-919            ║');
  console.log('╚════════════════════════════════════════════════════════════╝');
  ...
  const report = `# WOR-919 Smoke Test Report
...
`;
  ...
  writeFileSync('WOR-919-SMOKE-TEST-REPORT.md', report);
  console.log('\n📄 Report saved to WOR-919-SMOKE-TEST-REPORT.md');
