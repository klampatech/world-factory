/**
 * WOR-955 Smoke Test -- Full End-to-End
...
const REPORT_PATH = '/home/kyle/projects/world-generator/WOR-955-SMOKE-TEST-REPORT.md';
...
async function capture(page, prefix) {
  const file = screenshotFilename('WOR-955-' + prefix);
...
async function run() {
  ...
  // 1. POST /api/v1/worlds
  {
    var r = await apiCtx.post(API_BASE + '/worlds', {
      data: { name: 'WOR-955 Smoke Test World', seed: 955955, config: { genre: 'fantasy', era: 'medieval' } }
    });
...
function writeReport() {
  ...
  var md = '# WOR-955 Smoke Test Report\n\n';
  md += '**Date:** 2026-05-10\n';
