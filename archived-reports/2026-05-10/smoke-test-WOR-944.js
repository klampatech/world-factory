/**
 * WOR-944 Smoke Test -- Full End-to-End
 * Tests all 18 backend API endpoints + full frontend UI flow
 * against the running application on the main branch.
 * 
 * Latest commit: 44f3a79 fix(WOR-921): Use preview server with API proxy for frontend
 */
...
const REPORT_PATH = '/home/kyle/projects/world-generator/WOR-944-SMOKE-TEST-REPORT.md';
...
async function capture(page, prefix) {
  const file = screenshotFilename('WOR-944-' + prefix);
...
async function run() {
  ...
  // 1. POST /api/v1/worlds
  {
    var r = await apiCtx.post(API_BASE + '/worlds', {
      data: { name: 'WOR-944 Smoke Test World', seed: 944944, config: { genre: 'fantasy', era: 'medieval' } }
    });
  ...
  // 4. DELETE /api/v1/worlds/:id -- schedule for deletion
  {
  ...
  // 5. GET /api/v1/worlds/:id/planet -- use a recently-created world
  {
    var recentR = await apiCtx.get(API_BASE + '/worlds');
    var recentBody = await recentR.json().catch(function() { return {}; });
    var recentId = (recentBody.data && recentBody.data.worlds && recentBody.data.worlds[0] && recentBody.data.worlds[0].id) || '';
    if (recentId) {
  ...
  // 10. GET /api/v1/worlds/:id/figures/:figure_id
  {
    ...
    if (recentId) {
      ...
      if (figBody.data && figBody.data.figures && figBody.data.figures.length > 0) {
        figId = figBody.data.figures[0].id || '';
      }
    }
    if (figId) {
  ...
  // 18. GET /api/v1/worlds/:id/figures (second coverage for robustness)
  {
    var recentR = await apiCtx.get(API_BASE + '/worlds');
    var recentBody = await recentR.json().catch(function() { return {}; });
    var recentId = (recentBody.data && recentBody.data.worlds && recentBody.data.worlds[0] && recentBody.data.worlds[0].id) || '';
    if (recentId) {
      var r = await apiCtx.get(API_BASE + '/worlds/' + recentId.replace('world:', '') + '/figures');
      var ok = r.status() === 200;
      results.api.push({ name: 'GET /api/v1/worlds/:id/figures (alt)', path: '/api/v1/worlds/:id/figures', method: 'GET', status: r.status(), ok: ok });
      console.log('  GET /worlds/:id/figures (alt) -> ' + r.status() + ' ' + (ok ? 'PASS' : 'FAIL'));
    }
  }
  ...
  // World detail view -- navigate to a ready world directly by URL
  {
    var recentR = await apiCtx.get(API_BASE + '/worlds');
    var recentBody = await recentR.json().catch(function() { return {}; });
    var recentWorld = (recentBody.data && recentBody.data.worlds && recentBody.data.worlds[0]) || null;
    if (recentWorld && recentWorld.status === 'ready') {
      var worldId = recentWorld.id.replace('world:', '');
      await page.goto(FRONTEND_URL + '/worlds/world.html?id=world:' + worldId + '&tab=map', { waitUntil: 'networkidle', timeout: 15000 });
      await page.waitForTimeout(3000);
      await capture(page, '03-world-detail-ready');
      var ok = page.url().includes('/worlds/world.html');
      results.ui.push({ name: 'World detail view loads (ready world)', ok: ok, details: 'status=' + recentWorld.status });
      console.log('  World detail view (ready) -> ' + (ok ? 'PASS' : 'FAIL'));
    } else if (recentWorld) {
      results.ui.push({ name: 'World detail view loads (ready world)', ok: false, details: 'world status=' + (recentWorld.status || '?') + ', skipped' });
      console.log('  World detail view (ready) -> SKIP (world status=' + recentWorld.status + ')');
    } else {
      results.ui.push({ name: 'World detail view loads (ready world)', ok: false, details: 'no worlds available' });
      console.log('  World detail view (ready) -> SKIP (no worlds)');
    }
  }

  // Map canvas renders -- check on world detail page
  {
    await page.waitForTimeout(2000);
  ...
  // Map Voronoi polygons -- verify no scattered squares
  {
    var mapData = null;
    var recentR = await apiCtx.get(API_BASE + '/worlds');
    var recentBody = await recentR.json().catch(function() { return {}; });
  ...
  // World creation form
  {
    ...
    var form = await page.$('form, [class*="create"], [class*="new-world"]');
    var ok = !!form;
    results.ui.push({ name: 'World creation form accessible', ok: ok });
  ...
  if (errorCount > 0) {
    var genErrors = results.consoleErrors.filter(function(e) { return e.text && (e.text.includes('400') || e.text.includes('Bad Request') || e.text.includes('Failed to load timeline')); });
    if (genErrors.length > 0) {
      results.bugs.push({ title: 'Browser HTTP 400 errors on timeline', errors: genErrors });
    }
  }
...
function writeReport() {
  ...
  var md = '# WOR-944 Smoke Test Report\n\n';
  md += '**Date:** 2026-05-09\n';
  md += '**Commit:** 44f3a79 fix(WOR-921): Use preview server with API proxy for frontend\n';
