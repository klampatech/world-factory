#!/usr/bin/env node
/**
 * WOR-986: Smoke Test - All 18 API Endpoints + Frontend UI
 * Tests against running containers: backend:8080, frontend:9000
 */
...
async function runSmokeTest() {
  console.log('╔════════════════════════════════════════════════════════════╗');
  console.log('║            WOR-986: SMOKE TEST                               ║');
  console.log('║     All 18 API Endpoints + Frontend UI Verification         ║');
  console.log('╚════════════════════════════════════════════════════════════╝');
  console.log(`Started: ${new Date().toISOString()}\n`);

  let worldId = null;
  let worldName = `WOR-986-Test-${Date.now()}`;
  ...
  try {
    const res = await httpRequest(`${BACKEND_URL}/api/v1/worlds`, {
      ...
      body: JSON.stringify({ name: worldName, config: { genre: 'fantasy', seed: 986986 } })
    });
  ...
  // UI-03: API integration script loads
  try {
  ...
  // UI-04: Hex test page accessible
  try {
  ...
  console.log(`Total: ${passedCount}/${total} passed\n`);
  ...
  // Save log
  const logOutput = [
    `WOR-986 Smoke Test - ${new Date().toISOString()}`,
    `=====================================================`,
    ``,
    `API Endpoints: ${apiPassed}/${apiTotal} passed`,
  ...
  require('fs').writeFileSync('smoke-test-WOR-986-output.log', logOutput);
  console.log(`Log saved to: smoke-test-WOR-986-output.log`);
