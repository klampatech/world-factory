#!/usr/bin/env node
/**
 * WOR-1033: Smoke Test - Complete End-to-End Validation
...
const REPORT_FILE = 'WOR-1033-SMOKE-TEST-REPORT.md';
const SCREENSHOTS_DIR = 'screenshots/WOR-1033';
...
async function runSmokeTest() {
  console.log('╔════════════════════════════════════════════════════════════╗');
  console.log('║           WOR-1033: COMPLETE SMOKE TEST                    ║');
  ...
  let worldId = null;
  const worldName = `WOR-1033-Test-${Date.now()}`;
  ...
  try {
    const res = await httpRequest(`${BACKEND_URL}/api/v1/worlds`, {
      ...
      body: JSON.stringify({ name: worldName, config: { genre: 'fantasy', seed: 10331033 } })
    });
  ...
  try {
    const res = await httpRequest(`${BACKEND_URL}/api/v1/worlds`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: 'WOR-1033-Secondary-Test', config: { genre: 'fantasy', seed: 1033 } })
    });
    if (res.status === 201) {
  ...
  // UI-03: Frontend serves index.html
  try {
    const res = await httpRequest(`${FRONTEND_URL}/index.html`);
    const passed = res.status === 200;
    addResult('UI-03', 'Frontend serves index.html', passed, `HTTP ${res.status}`, `${FRONTEND_URL}/index.html`);
  } catch (e) {
    addResult('UI-03', 'Frontend serves index.html', false, e.message, `${FRONTEND_URL}/index.html`);
  }

  // UI-04: Frontend API proxy configured
  try {
    const res = await httpRequest(`${FRONTEND_URL}/api/v1/worlds`);
    // Frontend proxy returns backend data
    const passed = res.status === 200 || res.status === 502;
  ...
  // UI-05: Frontend world.html route
  try {
    const res = await httpRequest(`${FRONTEND_URL}/world.html`);
    const passed = res.status === 200;
    addResult('UI-05', 'Frontend world.html route', passed, `HTTP ${res.status}`, `${FRONTEND_URL}/world.html`);
  } catch (e) {
    addResult('UI-05', 'Frontend world.html route', false, e.message, `${FRONTEND_URL}/world.html`);
  }

  // UI-06: Frontend serves JS assets
  try {
    const res = await httpRequest(`${FRONTEND_URL}/js/api-integration.js`);
    const passed = res.status === 200;
    addResult('UI-06', 'Frontend serves JS assets', passed, `HTTP ${res.status}`, `${FRONTEND_URL}/js/api-integration.js`);
  } catch (e) {
    addResult('UI-06', 'Frontend serves JS assets', false, e.message, `${FRONTEND_URL}/js/api-integration.js`);
  }

  // UI-07: Frontend serves CSS
  try {
    const res = await httpRequest(`${FRONTEND_URL}/css/`);
    // May return 403 for directory, but assets should be accessible
    const passed = res.status === 200 || res.status === 403;
    addResult('UI-07', 'Frontend serves CSS assets', passed, `HTTP ${res.status}`, `${FRONTEND_URL}/css/`);
  } catch (e) {
    addResult('UI-07', 'Frontend serves CSS assets', false, e.message, `${FRONTEND_URL}/css/`);
  }
  ...
  // Generate report
  const report = `# WOR-1033: Smoke Test Report
