...
const SCREENSHOT_DIR = 'screenshots/smoke-test-WOR-835/';
const screenshots = [];
...
async function testApiEndpoints() {
  ...
  try {
    const createResponse = await fetch(`${BASE_URL}/api/v1/worlds`, {
      ...
      body: JSON.stringify({
        name: 'Smoke Test World 835',
        config: { width: 32, height: 32, pre_history_years: 50, seed: 12345 }
      })
    });
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (i === 29) console.log('⚠️ World still generating...\n');
  }
  ...
  // 9. GET /api/v1/worlds/:id/figures/:figure_id - Skip (no figures available)
  results.push({ endpoint: 'GET /api/v1/worlds/:id/figures/:figure_id', status: 'SKIP', pass: true, note: 'No figures to test' });
...
async function testFrontend(worldId) {
  ...
  try {
    ...
    if (await createBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      if (await nameInput.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        const submitBtn = page.locator('button:has-text("Submit"), button:has-text("Generate"), button:has-text("Create")').last();
  ...
  try {
    if (worldId) {
      await page.goto(`${BASE_URL}/worlds/${worldId}/map`);
      await page.waitForLoadState('networkidle');
      ...
      await page.goto(`${BASE_URL}/worlds/${worldId}/timeline`);
      await page.waitForLoadState('networkidle');
      await page.waitForTimeout(2000);
      ...
      await page.goto(`${BASE_URL}/worlds/${worldId}/dashboard`);
      await page.waitForLoadState('networkidle');
      await page.waitForTimeout(1000);
      ...
      await page.goto(`${BASE_URL}/worlds/${worldId}/figures`);
      await page.waitForLoadState('networkidle');
      await page.waitForTimeout(1000);
...
async function main() {
  console.log('╔════════════════════════════════════════════════════════════╗');
  console.log('║         WORLD FACTORY SMOKE TEST - WOR-835             ║');
  console.log('╚════════════════════════════════════════════════════════════╝');
  ...
  const report = `# WOR-835 Smoke Test Report
...
`;
  ...
  writeFileSync('WOR-835-SMOKE-TEST-REPORT.md', report);
  console.log('\n📄 Report saved to WOR-835-SMOKE-TEST-REPORT.md');
