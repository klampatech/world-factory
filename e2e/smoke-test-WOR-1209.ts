/**
 * WOR-1209: Comprehensive Smoke Test
...
async function runSmokeTest(): Promise<SmokeTestReport> {
  console.log('Starting WOR-1209 Comprehensive Smoke Test...');
  ...
  await test('API-01: POST /api/v1/worlds (Create World)', async () => {
    const resp = await context.request.post(`${BACKEND_URL}/api/v1/worlds`, {
      data: {
        name: `WOR-1209 Smoke Test ${Date.now()}`,
        seed: 77777,
        config: {
          genre: 'fantasy',
          era: 'medieval',
  ...
  await test('UI-13: Screenshot Capture', async () => {
    ...
    const filename = `screenshots/WOR-1209-${Date.now()}.png`;
...
runSmokeTest()
  .then(async (report) => {
    ...
    // Save report
    const fs = require('fs');
    const reportFile = `WOR-1209-SMOKE-TEST-REPORT.json`;
