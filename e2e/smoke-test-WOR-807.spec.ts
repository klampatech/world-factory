...
test.describe('WOR-807: Smoke Test', () => {
  let worldId: string;
  ...
  test.describe('Backend API Endpoints', () => {
    test('1. Health check', async () => {
      ...
      const result = await execAsync(`curl -s 'http://localhost:3000/health' -w "\\n%{http_code}"`);
      const lines = result.stdout.trim().split('\n');
      const status = parseInt(lines.pop() || '0');
      const bodyStr = lines.join('\n');
      const body = JSON.parse(bodyStr);
      
      console.log(`Health check status: ${status}, body: ${JSON.stringify(body)}`);
      
      expect(status).toBe(200);
      expect(body.status).toBe('ok');
    });

    test('2. POST /api/v1/worlds - Create world', async () => {
      const result = await apiRequest('POST', '/worlds', {
        name: 'WOR-807 Smoke Test World',
        seed: 807001,
    ...
    test('4. GET /api/v1/worlds/:id - Get specific world', async () => {
      if (!worldUuid) {
    ...
    test('9. GET /api/v1/worlds/:id/settlements - Get settlements', async () => {
      if (!worldUuid) {
        console.log('No worldUuid, skipping...');
        return;
      }
    ...
    test('10. GET /api/v1/worlds/:id/resources/summary - Get resources summary', async () => {
      if (!worldUuid) {
        console.log('No worldUuid, skipping...');
        return;
      }
    ...
    test('11. GET /api/v1/worlds/:id/artifacts - Get artifacts', async () => {
      if (!worldUuid) {
        console.log('No worldUuid, skipping...');
        return;
      }
    ...
    test('12. GET /api/v1/worlds/:id/timeline - Get timeline', async () => {
      if (!worldUuid) {
        console.log('No worldUuid, skipping...');
        return;
      }
      
      const result = await apiRequest('GET', `/worlds/${worldUuid}/timeline`);
      console.log(`Get timeline status: ${result.status}`);
      
      expect([200, 404]).toContain(result.status);
    });

    test('13. GET /api/v1/worlds/:id/events - Get events', async () => {
      if (!worldUuid) {
        console.log('No worldUuid, skipping...');
        return;
      }
      
      const result = await apiRequest('GET', `/worlds/${worldUuid}/events`);
      console.log(`Get events status: ${result.status}`);
  ...
  test.describe('Frontend UI Tests', () => {
    ...
    test('14. Frontend homepage loads', async ({ page }) => {
    ...
    test('15. World list page loads', async ({ page }) => {
    ...
    test('16. World detail page loads', async ({ page }) => {
    ...
    test('17. Tab navigation works', async ({ page }) => {
      await page.goto(`${FRONTEND_BASE}/world.html`);
      await page.waitForLoadState('networkidle', { timeout: 10000 });
