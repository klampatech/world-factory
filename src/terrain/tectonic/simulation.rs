...
use crate::world::entities::planet::{
    SubductionType, TectonicBoundary, TectonicBoundaryType, TectonicPlate, TectonicPlateType,
};
...
impl TectonicSimulator {
    ...
    pub fn simulate(&self) -> TectonicResult {
        ...
        let elevation_modifiers =
            self.calculate_elevation_modifiers(&cell_to_plate, &boundary_cells, &boundaries);
    ...
    fn generate_plate_seeds(&self) -> Vec<(f32, f32, TectonicPlateType)> {
        ...
        for i in 0..n {
            ...
            let x = (0.5
                + (radius * angle.cos() + noise_x * 0.2) * (self.config.width as f64 - 1.0))
                as f32;
            let y = (0.5
                + (radius * angle.sin() + noise_y * 0.2) * (self.config.height as f64 - 1.0))
                as f32;
    ...
    fn determine_plate_type(&self, x: f32, y: f32, _index: usize) -> TectonicPlateType {
        ...
        let continent_noise =
            self.noise
                .octave_noise_2d((x * 0.01) as f64, (y * 0.01) as f64, 4, 0.5, 2.0)
                as f32;
    ...
    fn assign_cells_to_plates(
        ...
        // Create plate IDs
        let plate_ids: Vec<Uuid> = seeds.iter().enumerate().map(|_| Uuid::new_v4()).collect();
    ...
    fn find_or_create_boundary(
        ...
        let (min_id, max_id) = if plate_a < plate_b {
            (*plate_a, *plate_b)
        } else {
            (*plate_b, *plate_a)
        };
                            ...
                            && ny < self.config.height as i32
                        {
                            ...
                            if let Some(cell_pairs) = boundary_cells.get(&neighbor_id) {
                                ...
                                if cell_pairs.iter().any(|(p1, p2)| {
                                    *p1 != boundary.plate_ids[0]
                                        && *p1 != boundary.plate_ids[1]
                                        && *p2 != boundary.plate_ids[0]
                                        && *p2 != boundary.plate_ids[1]
                                }) {
    ...
    fn build_plates(
        ...
        for (i, (sx, sy, plate_type)) in seeds.iter().enumerate() {
            ...
            let speed =
                (2.0 + self.noise.get(*sx as f64 * 0.02, *sy as f64 * 0.02).abs() * 10.0) as f32;

            let mut plate =
                TectonicPlate::new(plate_id, direction, speed, *plate_type, cells, area_km2);
    ...
    fn test_boundary_classification() {
        ...
        for boundary in &result.boundaries {
            match &boundary.boundary_type {
                TectonicBoundaryType::Divergent {
                    spreading_rate_cm_yr,
                } => assert!(*spreading_rate_cm_yr >= 0.0),
                TectonicBoundaryType::Convergent {
                    subduction_rate_cm_yr,
                    ..
                } => assert!(*subduction_rate_cm_yr >= 0.0),
                TectonicBoundaryType::Transform { slip_rate_cm_yr } => {
                    assert!(*slip_rate_cm_yr >= 0.0)
                }
                TectonicBoundaryType::Conservative { .. } => {}
            }
        }
