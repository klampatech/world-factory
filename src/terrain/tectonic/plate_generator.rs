...
use crate::world::entities::planet::{TectonicBoundary, TectonicPlate, TectonicPlateType};
...
impl PlateCellAllocator {
    ...
    fn grid_seeds(&self, count: usize) -> Vec<(f32, f32, TectonicPlateType)> {
        ...
        for i in 0..count {
            ...
            let x = (col as f64 + 0.5) * spacing_x as f64
                + self.noise.get(col as f64, row as f64) * spacing_x as f64 * 0.3;
            let y = (row as f64 + 0.5) * spacing_y as f64
                + self.noise.get(row as f64, col as f64) * spacing_y as f64 * 0.3;
    ...
    fn elevation_seeds(&self, count: usize) -> Vec<(f32, f32, TectonicPlateType)> {
        ...
        for i in 0..samples {
            ...
            let continent_score =
                self.noise.octave_noise_2d(nx * 2.0, ny * 2.0, 4, 0.5, 2.0) as f32;
    ...
    fn determine_type(&self, nx: f64, ny: f64, _index: usize) -> TectonicPlateType {
        ...
        let continent_score = self.noise.octave_noise_2d(nx * 3.0, ny * 3.0, 3, 0.5, 2.0) as f32;
