...
impl ErosionSimulator {
    ...
    pub fn apply(&self, grid: &mut TerrainGrid) {
        ...
        log::info!(
            "Erosion: running {} iterations on {}x{} grid",
            actual_iterations,
            width,
            height
        );
        ...
        if self.config.thermal_weathering {
            log::info!(
                "Erosion: applying thermal weathering ({} iterations)",
    ...
    fn erode_droplet(&self, grid: &mut TerrainGrid, width: u32, height: u32, seed: usize) {
        ...
        for _step in 0..max_steps {
            ...
            if cell_x < 1 || cell_x >= width as i32 - 1 || cell_y < 1 || cell_y >= height as i32 - 1
            {
                break;
            }
            ...
            if len > 0.001 {
                ...
                let angle =
                    self.noise.get_float(seed as f32 + _step as f32) * std::f32::consts::TAU;
                dir_x = angle.cos();
                dir_y = angle.sin();
            }
            ...
            let speed = len;
            let capacity = (-height_diff.max(self.config.min_slope)
                * self.config.sediment_capacity)
    ...
    fn interpolate_height(
        ...
        let h00 = grid
            ...
            .unwrap_or(0.0);
        let h10 = grid
            .get(x1 as u32, y0 as u32)
            .map(|c| c.height())
            .unwrap_or(0.0);
        let h01 = grid
            .get(x0 as u32, y1 as u32)
            .map(|c| c.height())
            .unwrap_or(0.0);
        let h11 = grid
            .get(x1 as u32, y1 as u32)
            .map(|c| c.height())
            .unwrap_or(0.0);
...
#[cfg(test)]
mod tests {
    use super::super::{TerrainConfig, TerrainGenerator, TerrainLayer};
    use super::*;
