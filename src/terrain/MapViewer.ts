...
import type { MapData, MapPolygon, Biome, ResourceLocation, GeographicEntity } from '../terrain/MapData';
...
export class MapViewer {
  ...
  private getPolygonColor(type: MapPolygon['type']): string {
    const colors: Record<MapPolygon['type'], string> = {
      territory: '#ffd700',
      biome: '#00ff88',
      region: '#00aaff',
      resource: '#ff6b6b',
  ...
  private getEntityColor(type: GeographicEntity['type']): string {
    const colors: Record<GeographicEntity['type'], string> = {
      city: '#ff4500',
      settlement: '#ff6b35',
      landmark: '#00ced1',
      fortress: '#8b0000',
