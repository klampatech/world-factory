...
use super::elevation::{PolygonGraph, Polygon};
...
impl ClimateCalculator {
        ...
        F: Fn(u32) -> f32 + Send + Sync,
    {
        ...
        let rain_shadow_map = self.calculate_rain_shadows(graph, &polygon_climates, latitude_fn, seed);
        ...
        log::debug!("Climate calculation complete: {} polygons processed", graph.len());
    }
        ...
        for i in 0..n {
            ...
            for dist in 1..=self.config.max_rain_shadow_distance {
                ...
                for j in 0..n {
                    ...
                    let other_centroid = self.get_polygon_centroid_estimate(other, j as u32, &latitude_fn);
                    ...
                    if dist_to_current < 0.01 { continue; }
            ...
            if mountain_height > 0.2 && distance_to_mountain < self.config.max_rain_shadow_distance {
                let shadow_strength = mountain_height * (1.0 - distance_to_mountain as f32 / self.config.max_rain_shadow_distance as f32);
...
mod tests {
    ...
    fn test_wind_direction_zones() {
        ...
        assert!(matches!(WindDirection::from_latitude(15.0), WindDirection::Easterly));
        
        // Westerlies in temperate
        assert!(matches!(WindDirection::from_latitude(45.0), WindDirection::Westerly));
        
        // Polar easterlies
        assert!(matches!(WindDirection::from_latitude(75.0), WindDirection::PolarEasterly));
    }
    ...
    fn test_moisture_coastal_effect() {
        ...
        assert!(moist_near > moist_far, "Closer to coast should be more moist");
    }
    ...
    fn test_rain_shadow_effect() {
        ...
        assert!(moist_shadow < moist_clear, "Rain shadow should reduce moisture");
    }
    ...
    fn test_deterministic_temperature() {
        ...
        assert_eq!(temp1, temp2, "Temperature calculation should be deterministic");
    }
