...
use super::{EventContext, ProbabilityConfig, ProbabilityFactor, ProbabilityResult, Season};
...
impl ProbabilityEngine {
    ...
    fn initialize_base_probabilities(&mut self) {
        ...
        self.base_probabilities
            .insert(EventType::SettlementFounded, 0.0002); // ~1 per 5000 years per region
        self.base_probabilities
            .insert(EventType::PopulationGrowth, 0.01); // Continuous process
        self.base_probabilities.insert(EventType::Migration, 0.001); // ~1 per 1000 years
        self.base_probabilities
            .insert(EventType::Immigration, 0.0008);
        self.base_probabilities.insert(EventType::Festival, 0.002); // ~1 per 500 years
        self.base_probabilities
            .insert(EventType::TradeRouteEstablished, 0.0003);
        self.base_probabilities.insert(EventType::Discovery, 0.0005);

        // Medium base probability events
        self.base_probabilities
            .insert(EventType::WarDeclared, 0.0001); // ~1 per 10000 years (rare but impactful)
        ...
        self.base_probabilities
            .insert(EventType::Earthquake, 0.0003);
        ...
        self.base_probabilities
            .insert(EventType::Exploration, 0.0004);
        self.base_probabilities.insert(EventType::Treaty, 0.0002);
        self.base_probabilities
            .insert(EventType::AllianceFormed, 0.0002);
        self.base_probabilities
            .insert(EventType::AllianceBroken, 0.0002);
        self.base_probabilities
            .insert(EventType::Succession, 0.0008);
        self.base_probabilities
            .insert(EventType::GovernmentReform, 0.0001);
        self.base_probabilities
            .insert(EventType::CulturalAchievement, 0.0003);
        self.base_probabilities
            .insert(EventType::ReligiousEvent, 0.0004);
        self.base_probabilities.insert(EventType::Invention, 0.0002);
        self.base_probabilities
            .insert(EventType::LawEnacted, 0.0003);

        // Phase 2 Event Types
        self.base_probabilities
            .insert(EventType::SocietyFormed, 0.00005); // ~1 per 20000 years
        self.base_probabilities
            .insert(EventType::FigureRises, 0.001); // ~1 per 1000 years
        self.base_probabilities.insert(EventType::FigureDies, 0.001); // ~1 per 1000 years
        self.base_probabilities
            .insert(EventType::ArtifactCreated, 0.0002); // ~1 per 5000 years
        self.base_probabilities
            .insert(EventType::ArtifactActivated, 0.00001); // ~1 per 100000 years (rare & dangerous)

        // Low base probability events (major occurrences)
        self.base_probabilities
            .insert(EventType::NationFounded, 0.00002); // ~1 per 50000 years
        self.base_probabilities.insert(EventType::Coup, 0.00005);
        self.base_probabilities
            .insert(EventType::GoldenAge, 0.00003);
        self.base_probabilities
            .insert(EventType::ReligiousReformation, 0.00002);
        self.base_probabilities
            .insert(EventType::FirstContact, 0.00001);
        self.base_probabilities
            .insert(EventType::ResourceDiscovery, 0.0003);
        ...
        self.base_probabilities
            .insert(EventType::MonumentCompleted, 0.0001);
        self.base_probabilities.insert(EventType::Collapse, 0.00001);
        self.base_probabilities
            .insert(EventType::Destruction, 0.0002);
        self.base_probabilities
            .insert(EventType::MeteorStrike, 0.000001); // Extremely rare
        self.base_probabilities
            .insert(EventType::Extinction, 0.000002);
        self.base_probabilities
            .insert(EventType::MagicalCatastrophe, 0.00001);
    }
    ...
    pub fn calculate_event_probability(
        ...
        let random_seed = self.hash_values(
            self.seed,
            event_type.name().len() as u64,
            current_year as u64,
        );
    ...
    fn calculate_historical_modifier(
        &self,
        event_type: EventType,
        context: &EventContext,
        current_year: i32,
        ...
        match event_type {
            ...
            EventType::PopulationGrowth => {
                ...
                let has_recent_loss = context.recent_events.iter().any(|e| {
                    matches!(
                        e.event_type,
                        EventType::Plague | EventType::Famine | EventType::WarDeclared
                    ) && e.year > current_year - 50
                });
...
mod tests {
    ...
    fn test_base_probability_lookup() {
        ...
        assert!(
            engine.get_base_probability(EventType::MeteorStrike)
                < engine.get_base_probability(EventType::Festival)
        );
    }
    ...
    fn test_probability_calculation() {
        ...
        let result =
            engine.calculate_event_probability(EventType::SettlementFounded, &context, 1000);
    ...
    fn test_environmental_modifier() {
        ...
        let drought_prob_desert =
            engine.calculate_event_probability(EventType::Drought, &desert_context, 1000);

        let drought_prob_forest =
            engine.calculate_event_probability(EventType::Drought, &forest_context, 1000);

        assert!(
            drought_prob_desert.environmental_modifier > drought_prob_forest.environmental_modifier
        );
    }
    ...
    fn test_war_state_modifier() {
        ...
        let battle_peaceful =
            engine.calculate_event_probability(EventType::Battle, &peaceful, 1000);

        let battle_war = engine.calculate_event_probability(EventType::Battle, &at_war, 1000);
    ...
    fn test_cooldown_tracking() {
        ...
        let prob_immediate =
            engine.calculate_event_probability(EventType::WarDeclared, &context, 1000);
        assert!(prob_immediate.historical_modifier < 0.5);

        // 50 years later
        let prob_later = engine.calculate_event_probability(EventType::WarDeclared, &context, 1050);
    ...
    fn test_determinism() {
        ...
        let result1 = engine1.calculate_event_probability(EventType::Battle, &context, 1500);

        let result2 = engine2.calculate_event_probability(EventType::Battle, &context, 1500);
    ...
    fn test_different_seeds_different_results() {
        ...
        let result1 = engine1.calculate_event_probability(EventType::Discovery, &context, 1200);

        let result2 = engine2.calculate_event_probability(EventType::Discovery, &context, 1200);
    ...
    fn test_figure_influence_modifier() {
        ...
        let scholar_result =
            engine.calculate_event_probability(EventType::Discovery, &scholar_context, 1000);
        ...
        assert!(
            scholar_boost > 1.35 && scholar_boost <= 1.45,
            "Scholar should boost Discovery by ~40%, got {}",
            scholar_boost
        );
        ...
        let hero_result =
            engine.calculate_event_probability(EventType::HeroicAct, &hero_context, 1000);
        ...
        assert!(
            hero_boost > 1.35,
            "Hero should boost HeroicAct by ~40%, got {}",
            hero_boost
        );
        ...
        let explorer_result =
            engine.calculate_event_probability(EventType::Exploration, &explorer_context, 1000);
        ...
        assert!(
            explorer_boost > 1.30,
            "Explorer should boost Exploration by ~35%, got {}",
            explorer_boost
        );
        ...
        let warlord_result =
            engine.calculate_event_probability(EventType::WarDeclared, &warlord_context, 1000);
        ...
        assert!(
            warlord_boost > 1.25,
            "MilitaryLeader should boost WarDeclared by ~30%, got {}",
            warlord_boost
        );
    }
}
