...
impl EffectApplicator {
    ...
    pub fn apply_event_effects(&mut self, event: &Event) -> EffectApplicationResult {
    ...
    pub fn remove_entity(&mut self, entity_id: Uuid) {
        self.entity_cache
            .insert(entity_id, EntityStatus::DoesNotExist);
    }

    /// Track state changes from an effect.
    fn track_effect_changes(&mut self, effect: &EventEffect, result: &mut EffectApplicationResult) {
        match effect {
...
impl EffectApplicator {
    ...
    pub fn calculate_effect_magnitude(
        significance: f32,
        base_magnitude: EffectMagnitude,
    ) -> EffectMagnitude {
...
impl EffectApplicator {
    ...
    pub fn border_shift_to_effect(from: Option<Uuid>, to: Uuid, territory: Uuid) -> EventEffect {
        EventEffect::BorderShift {
            from,
            to,
            territory,
...
mod tests {
    ...
    fn test_effect_priority_ordering() {
        ...
        assert!(
            EffectApplicator::effect_priority(&population_effect)
    ...
    fn test_magnitude_application() {
        assert_eq!(
            EffectApplicator::apply_magnitude(100.0, EffectMagnitude::Minor),
            50.0
        );
        assert_eq!(
            EffectApplicator::apply_magnitude(100.0, EffectMagnitude::Moderate),
            100.0
        );
        assert_eq!(
            EffectApplicator::apply_magnitude(100.0, EffectMagnitude::Major),
            200.0
        );
        assert_eq!(
            EffectApplicator::apply_magnitude(100.0, EffectMagnitude::Catastrophic),
            500.0
        );
    }
}
