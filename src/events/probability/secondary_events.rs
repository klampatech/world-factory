...
use crate::events::{Event, EventType, EventEffect, EventBuilder};
        ...
        for rule in &self.rules {
            ...
            for effect in &event.effects {
                if effect.effect_name() == rule.trigger_effect {
                    ...
                    let prob = self.calculate_trigger_probability(rule.base_probability, &event.id.id);
...
mod tests {
    ...
    fn test_default_rules_exist() {
        ...
        let rule_names: Vec<_> = rules.iter()
            .map(|r| r.trigger_effect.as_str())
            .collect();
