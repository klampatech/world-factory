...
use super::{Event, EventCategory, EventStore, EventType};
...
impl EventTimeline {
    ...
    pub fn during_year(&self, year: i32) -> Vec<&Event> {
        self.events
            ...
            .filter(|e| {
                ...
                let end = e.end_time.as_ref().map(|t| t.get_year()).unwrap_or(start);
    ...
    pub fn stats(&self) -> TimelineStats {
        let mut by_type: std::collections::HashMap<EventType, usize> =
            std::collections::HashMap::new();
        let mut by_category: std::collections::HashMap<EventCategory, usize> =
            std::collections::HashMap::new();
    ...
    pub fn top_events(&self, n: usize) -> Vec<&Event> {
        ...
        sorted.sort_by(|a, b| {
            b.significance
                .unwrap()
                .partial_cmp(&a.significance.unwrap())
                .unwrap()
        });
