...
impl Society {
    ...
    pub fn remove_settlement(&mut self, settlement_id: Uuid) -> bool {
        if let Some(pos) = self.settlement_ids.iter().position(|&id| id == settlement_id) {
    ...
    pub fn record_population(&mut self, year: i32, population: u64) {
        self.population_history.push(PopulationSample { year, population });
...
impl SocietyRegistry {
    ...
    pub fn total_settlements(&self) -> usize {
        self.societies.values().map(|s| s.settlement_ids.len()).sum()
    }
    
    /// Form initial societies from settlements.
    /// Each settlement starts as a Band, upgrades to Tribe at 50 pop.
    pub fn form_initial_societies(
        &mut self,
        settlements: &[(Uuid, String, SpeciesId, u64, i32)],
    ) {
