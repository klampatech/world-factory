...
impl ArtifactRarity {
    ...
    pub fn from_significance(significance: f32) -> Self {
        if significance >= 0.95 { Self::Mythic }
        else if significance >= 0.85 { Self::Legendary }
        else if significance >= 0.7 { Self::Rare }
        else if significance >= 0.5 { Self::Uncommon }
        else { Self::Common }
    }
...
impl CataclysmTriggerSystem {
    ...
    pub fn calculate_annual_probability(
        ...
        if roll < capped_probability {
            Some((capped_probability, highest_rarity, triggering_artifact_id.unwrap_or(Uuid::nil())))
        } else {
            None
        }
    }
...
impl ArtifactCreationCondition {
    ...
    pub fn is_satisfied(&self, context: &ArtifactCreationContext) -> bool {
        ...
        match self.condition_type {
            ArtifactCreationConditionType::SignificantEvent => {
                context.related_event.is_some()
            }
            ArtifactCreationConditionType::NotableFigure => {
                context.creator_figure_id.is_some() && 
                self.required_figure_type.as_ref().map_or(true, |t| {
                    context.creator_figure_type.as_ref() == Some(t)
                })
            }
            ArtifactCreationConditionType::RareResources => {
                context.uses_rare_resources
            }
            ...
            ArtifactCreationConditionType::Competition => {
                context.was_competition_winner
            }
            ArtifactCreationConditionType::DarkRitual => {
                context.used_dark_ritual || context.has_cursed_property
            }
            ArtifactCreationConditionType::Bloodline => {
                context.in_bloodline
            }
        }
    }
...
impl ArtifactStore {
    ...
    pub fn by_category(&self, category: ArtifactCategory) -> Vec<&Artifact> {
        self.artifacts.iter().filter(|a| a.category == category).collect()
    }
...
mod tests {
    ...
    fn test_rarity_cataclysm_probability() {
        ...
        assert!(ArtifactRarity::Rare.cataclysm_probability() > ArtifactRarity::Uncommon.cataclysm_probability());
        assert!(ArtifactRarity::Legendary.cataclysm_probability() > ArtifactRarity::Rare.cataclysm_probability());
        assert!(ArtifactRarity::Mythic.cataclysm_probability() > ArtifactRarity::Legendary.cataclysm_probability());
    }
    ...
    fn test_artifact_effects() {
        ...
        for effect in &effects {
            ...
            if effect.effect_type != ArtifactEffectType::Cursed && effect.effect_type != ArtifactEffectType::Doom {
    ...
    fn test_creation_conditions() {
        ...
        let crown_conditions = ArtifactCreationCondition::default_for_category(ArtifactCategory::CrownJewel);
        assert!(!crown_conditions.is_empty());
        
        // Test Weapon conditions
        let weapon_conditions = ArtifactCreationCondition::default_for_category(ArtifactCategory::Weapon);
    ...
    fn test_artifact_potential_cataclysm_type() {
        ...
        let sacred = Artifact::new(
            world_id, "Holy Relic".to_string(), ArtifactCategory::Sacred, 1000,
            "A sacred relic".to_string(), 0.8,
        );
        assert_eq!(sacred.potential_cataclysm_type_name(), "divine_wrath");
        
        let magical = Artifact::new(
            world_id, "Magic Orb".to_string(), ArtifactCategory::Magical, 1000,
            "A magical orb".to_string(), 0.8,
        );
        assert_eq!(magical.potential_cataclysm_type_name(), "magical_cataclysm");
        
        let weapon = Artifact::new(
            world_id, "Doom Blade".to_string(), ArtifactCategory::Weapon, 1000,
            "A cursed weapon".to_string(), 0.8,
        );
        assert_eq!(weapon.potential_cataclysm_type_name(), "civilizational_collapse");
    }
}
