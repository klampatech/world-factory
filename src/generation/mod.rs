...
impl WorldGenerator {
    ...
    pub fn generate(&self, seed: u64) -> GeneratedWorld {
        ...
        let wonders = if let Some(ref wonder_config) = self.config.wonders {
            ...
            let terrain_data =
                crate::terrain::natural_wonders::TerrainDataForSpawning::from_elevation_grid(
                    &elevation,
...
mod tests {
    ...
    fn test_natural_wonders_generation() {
        ...
        assert!(
            !world.wonders.is_empty(),
            "World should have natural wonders"
        );
        ...
        for wonder in &world.wonders {
            ...
            assert!(
                wonder.x >= 0.0 && wonder.x <= 64.0,
                "Wonder x should be within world bounds"
            );
            assert!(
                wonder.y >= 0.0 && wonder.y <= 64.0,
                "Wonder y should be within world bounds"
            );
            assert!(
                !wonder.bonuses.is_empty(),
                "Wonder should have at least one bonus"
            );
        }
