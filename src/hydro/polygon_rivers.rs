...
mod tests {
    ...
    fn test_confluence_detection() {
        ...
        // Check that at least one river has a confluence at polygon 102
        let has_confluence = rivers
            .iter()
            .any(|r| r.confluences.iter().any(|c| c.polygon_id == 102));
        assert!(
            has_confluence,
            "At least one river should have a confluence at polygon 102"
        );
    }
