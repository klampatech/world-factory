...
use crate::util::{Direction, Rng, Vec2};
            ...
            if let Some(river) = self.trace_river(elevation, sea_level, source, rng) {
                ...
                if river.length >= self.config.min_length {
                    ...
                    let overlaps = self
                        .rivers
                        .iter()
                        .any(|r| river.path.iter().any(|p| r.cells.contains(p)));
    ...
    fn find_source_points(&mut self, elevation: &ElevationGrid, rng: &mut Rng) -> Vec<Vec2<i32>> {
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                if elev >= self.config.source_elevation
                    && flow >= self.config.accumulation_threshold
                {
        ...
        while path.len() < self.config.max_length as usize {
            ...
            match best_next {
                Some((next, slope)) => {
                    ...
                    visited.insert(next);
                    min_elevation =
                        min_elevation.min(elevation.get_value_unchecked(next.x, next.y));
                    ...
                    if slope < 0.01 {
                        ...
                        if rng.random_float() < 0.2 {
                            path.push(next);
                            return Some(self.create_river(
                                path,
                                flow_rate,
                                DrainTarget::Lake(next),
                            ));
                        }
                    }
                }
...
mod tests {
    ...
    fn test_deterministic_rivers() {
        ...
        assert_eq!(
            rivers1.len(),
            rivers2.len(),
            "Same seed should produce same number of rivers"
        );
        ...
        for (r1, r2) in rivers1.iter().zip(rivers2.iter()) {
            assert_eq!(
                r1.path.len(),
                r2.path.len(),
                "River paths should be same length"
            );
        }
    }
